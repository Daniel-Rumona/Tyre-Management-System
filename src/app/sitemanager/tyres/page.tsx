'use client'

import React, { useState } from 'react'
import { 
  Card, 
  Table, 
  Tag, 
  Button, 
  Input, 
  Select, 
  Space, 
  Typography,
  Statistic,
  Row,
  Col,
  Progress,
  Tabs,
  Drawer,
  Descriptions,
  List,
  Avatar,
  Divider,
  Badge
} from 'antd'
import { 
  SearchOutlined, 
  BarChartOutlined,
  PieChartOutlined,
  CheckCircleOutlined,
  CloseCircleOutlined,
  WarningOutlined,
  CaretRightOutlined,
  DownloadOutlined,
  FilterOutlined
} from '@ant-design/icons'

const { Title, Text } = Typography
const { TabPane } = Tabs
const { Option } = Select

export default function SiteManagerTyres() {
  const [searchText, setSearchText] = useState('')
  const [statusFilter, setStatusFilter] = useState<string | null>(null)
  const [siteFilter, setUserSiteFilter] = useState<string | null>(null)
  const [drawerVisible, setDrawerVisible] = useState(false)
  const [selectedTyre, setSelectedTyre] = useState<any>(null)
  const [activeTab, setActiveTab] = useState('1')

  // Sample sites
  const sites = ['North Pit', 'South Pit', 'Main Workshop']

  // Sample data for tyres
  const tyres = [
    {
      id: 'T001',
      serial: 'SN123456',
      brand: 'Michelin',
      model: 'X Mine D2',
      size: '27.00R49',
      location: 'North Pit',
      status: 'In Service',
      machine: 'Haul Truck 01',
      position: 'Front Left',
      installDate: '2023-01-15',
      treadDepth: 78,
      hours: 1200,
      purchaseDate: '2022-12-10',
      cost: 12500,
      history: [
        { date: '2023-01-15', event: 'Installation', details: 'Installed on Haul Truck 01, Front Left' },
        { date: '2023-03-10', event: 'Inspection', details: 'Tread depth: 85%, Pressure adjusted' },
        { date: '2023-05-20', event: 'Inspection', details: 'Tread depth: 78%, No issues detected' }
      ]
    },
    {
      id: 'T002',
      serial: 'SN234567',
      brand: 'Bridgestone',
      model: 'VSDL',
      size: '27.00R49',
      location: 'Main Workshop',
      status: 'In Stock',
      machine: '',
      position: '',
      installDate: '',
      treadDepth: 100,
      hours: 0,
      purchaseDate: '2023-02-05',
      cost: 13200,
      history: [
        { date: '2023-02-10', event: 'Received', details: 'Added to inventory' },
        { date: '2023-04-05', event: 'Inspection', details: 'Quality check passed' }
      ]
    },
    {
      id: 'T003',
      serial: 'SN345678',
      brand: 'Goodyear',
      model: 'RT-4A',
      size: '24.00R35',
      location: 'North Pit',
      status: 'In Service',
      machine: 'Loader 03',
      position: 'Rear Right',
      installDate: '2022-11-20',
      treadDepth: 45,
      hours: 2100,
      purchaseDate: '2022-09-15',
      cost: 9800,
      history: [
        { date: '2022-11-20', event: 'Installation', details: 'Installed on Loader 03, Rear Right' },
        { date: '2023-01-15', event: 'Inspection', details: 'Tread depth: 72%, Minor cuts observed' },
        { date: '2023-03-25', event: 'Inspection', details: 'Tread depth: 58%, Wear accelerating' },
        { date: '2023-05-30', event: 'Inspection', details: 'Tread depth: 45%, Scheduled for rotation' }
      ]
    },
    {
      id: 'T004',
      serial: 'SN456789',
      brand: 'Michelin',
      model: 'X Mine D2',
      size: '27.00R49',
      location: 'South Pit',
      status: 'Needs Replacement',
      machine: 'Haul Truck 05',
      position: 'Rear Left',
      installDate: '2022-08-10',
      treadDepth: 12,
      hours: 3200,
      purchaseDate: '2022-06-20',
      cost: 12500,
      history: [
        { date: '2022-08-10', event: 'Installation', details: 'Installed on Haul Truck 05, Rear Left' },
        { date: '2022-10-20', event: 'Inspection', details: 'Tread depth: 80%, No issues detected' },
        { date: '2023-01-05', event: 'Inspection', details: 'Tread depth: 60%, Normal wear pattern' },
        { date: '2023-03-15', event: 'Inspection', details: 'Tread depth: 30%, Accelerated wear noted' },
        { date: '2023-06-01', event: 'Inspection', details: 'Tread depth: 12%, Replacement required' }
      ]
    },
    {
      id: 'T005',
      serial: 'SN567890',
      brand: 'Bridgestone',
      model: 'VRDP',
      size: '24.00R35',
      location: 'South Pit',
      status: 'Under Repair',
      machine: 'Formerly on Loader 02',
      position: 'Previously Front Right',
      installDate: '2022-09-15',
      treadDepth: 55,
      hours: 1800,
      purchaseDate: '2022-07-10',
      cost: 10200,
      history: [
        { date: '2022-09-15', event: 'Installation', details: 'Installed on Loader 02, Front Right' },
        { date: '2022-12-10', event: 'Inspection', details: 'Tread depth: 85%, No issues detected' },
        { date: '2023-03-05', event: 'Inspection', details: 'Tread depth: 70%, Normal wear pattern' },
        { date: '2023-05-25', event: 'Damage Report', details: 'Sidewall puncture detected' },
        { date: '2023-05-26', event: 'Removal', details: 'Removed for repair' }
      ]
    }
  ]

  // Site-specific statistics
  const siteInventory = sites.map(site => {
    const siteTyres = tyres.filter(t => t.location === site)
    return {
      site,
      total: siteTyres.length,
      inService: siteTyres.filter(t => t.status === 'In Service').length,
      inStock: siteTyres.filter(t => t.status === 'In Stock').length,
      needsReplacement: siteTyres.filter(t => t.status === 'Needs Replacement').length,
      underRepair: siteTyres.filter(t => t.status === 'Under Repair').length,
      totalValue: siteTyres.reduce((sum, t) => sum + t.cost, 0)
    }
  })

  // Filter tyres based on search and filters
  const filteredTyres = tyres.filter(tyre => {
    // Filter by search text
    const matchesSearch = 
      tyre.serial.toLowerCase().includes(searchText.toLowerCase()) ||
      tyre.brand.toLowerCase().includes(searchText.toLowerCase()) ||
      tyre.model.toLowerCase().includes(searchText.toLowerCase()) ||
      tyre.machine.toLowerCase().includes(searchText.toLowerCase())
    
    // Filter by status
    const matchesStatus = statusFilter ? tyre.status === statusFilter : true
    
    // Filter by site
    const matchesSite = siteFilter ? tyre.location === siteFilter : true
    
    return matchesSearch && matchesStatus && matchesSite
  })

  // Handle opening the tyre details drawer
  const handleViewTyre = (tyre: any) => {
    setSelectedTyre(tyre)
    setDrawerVisible(true)
  }

  // Get status color based on tyre status
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'In Service':
        return 'green'
      case 'In Stock':
        return 'blue'
      case 'Under Repair':
        return 'orange'
      case 'Needs Replacement':
        return 'red'
      default:
        return 'default'
    }
  }

  // Get condition color based on tread depth
  const getConditionColor = (treadDepth: number) => {
    if (treadDepth >= 70) return 'green'
    if (treadDepth >= 40) return 'orange'
    return 'red'
  }

  // Columns configuration for the tyres table
  const columns = [
    {
      title: 'Serial',
      dataIndex: 'serial',
      key: 'serial',
    },
    {
      title: 'Brand/Model',
      key: 'brand',
      render: (tyre: any) => `${tyre.brand} ${tyre.model}`
    },
    {
      title: 'Size',
      dataIndex: 'size',
      key: 'size',
    },
    {
      title: 'Location',
      dataIndex: 'location',
      key: 'location',
      filters: sites.map(site => ({ text: site, value: site })),
      onFilter: (value: any, record: any) => record.location === value
    },
    {
      title: 'Machine',
      dataIndex: 'machine',
      key: 'machine',
      render: (machine: string) => machine || '-'
    },
    {
      title: 'Condition',
      key: 'condition',
      render: (tyre: any) => (
        <Progress 
          percent={tyre.treadDepth} 
          size="small" 
          status={tyre.treadDepth < 20 ? "exception" : undefined}
          strokeColor={getConditionColor(tyre.treadDepth)}
        />
      )
    },
    {
      title: 'Status',
      dataIndex: 'status',
      key: 'status',
      render: (status: string) => (
        <Tag color={getStatusColor(status)}>
          {status}
        </Tag>
      )
    },
    {
      title: 'Action',
      key: 'action',
      render: (_: any, tyre: any) => (
        <Button type="primary" onClick={() => handleViewTyre(tyre)}>
          View Details
        </Button>
      )
    }
  ]

  // Calculate financial and inventory statistics
  const totalValue = tyres.reduce((sum, tyre) => sum + tyre.cost, 0)
  const activeValue = tyres.filter(t => t.status === 'In Service').reduce((sum, tyre) => sum + tyre.cost, 0)
  const inventoryStats = {
    total: tyres.length,
    inService: tyres.filter(t => t.status === 'In Service').length,
    inStock: tyres.filter(t => t.status === 'In Stock').length,
    underRepair: tyres.filter(t => t.status === 'Under Repair').length,
    needsReplacement: tyres.filter(t => t.status === 'Needs Replacement').length,
    totalValue,
    activeValue,
    inactiveValue: totalValue - activeValue
  }

  return (
    <div className="site-manager-tyres">
      <Tabs 
        activeKey={activeTab} 
        onChange={setActiveTab} 
        className="mb-4"
        size="large"
        style={{ background: '#fff' }}
      >
        <TabPane tab="Fleet Overview" key="1">
          {/* Fleet statistics */}
          <Row gutter={[16, 16]} className="mb-6">
            <Col xs={24} sm={12} md={8} lg={6}>
              <Card bodyStyle={{ padding: '20px' }}>
                <Statistic 
                  title="Total Tyres" 
                  value={inventoryStats.total} 
                  valueStyle={{ color: '#1677ff' }}
                  prefix={<PieChartOutlined />}
                />
              </Card>
            </Col>
            <Col xs={24} sm={12} md={6}>
              <Card>
                <Statistic 
                  title="In Service" 
                  value={inventoryStats.inService} 
                  valueStyle={{ color: '#52c41a' }}
                  prefix={<CheckCircleOutlined />}
                />
              </Card>
            </Col>
            <Col xs={24} sm={12} md={6}>
              <Card>
                <Statistic 
                  title="Needs Replacement" 
                  value={inventoryStats.needsReplacement} 
                  valueStyle={{ color: '#ff4d4f' }}
                  prefix={<CloseCircleOutlined />}
                />
              </Card>
            </Col>
            <Col xs={24} sm={12} md={6}>
              <Card>
                <Statistic 
                  title="Fleet Value" 
                  value={inventoryStats.totalValue} 
                  valueStyle={{ color: '#1677ff' }}
                  prefix="$"
                  precision={0}
                />
              </Card>
            </Col>
          </Row>
          
          {/* Filters and search */}
          <Card className="mb-6" bodyStyle={{ padding: '20px' }}>
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                <Input
                  placeholder="Search by serial, brand, model, or machine"
                  prefix={<SearchOutlined />}
                  value={searchText}
                  onChange={e => setSearchText(e.target.value)}
                  allowClear
                  size="large"
                />
              </div>
              <div className="flex gap-2 flex-wrap">
                <Select
                  placeholder="Filter by location"
                  style={{ width: 160 }}
                  onChange={value => setUserSiteFilter(value)}
                  allowClear
                  value={siteFilter}
                  size="large"
                >
                  {sites.map(site => (
                    <Option key={site} value={site}>{site}</Option>
                  ))}
                </Select>
                <Select
                  placeholder="Filter by status"
                  style={{ width: 180 }}
                  onChange={value => setStatusFilter(value)}
                  allowClear
                  value={statusFilter}
                  size="large"
                >
                  <Option value="In Service">In Service</Option>
                  <Option value="In Stock">In Stock</Option>
                  <Option value="Under Repair">Under Repair</Option>
                  <Option value="Needs Replacement">Needs Replacement</Option>
                </Select>
              </div>
            </div>
          </Card>
          
          {/* Actions toolbar */}
          <div className="flex justify-between items-center mb-6">
            <div>
              <Text>Showing {filteredTyres.length} of {tyres.length} tyres</Text>
            </div>
            <Space size="middle">
              <Button icon={<DownloadOutlined />} size="large">Export</Button>
              <Button icon={<FilterOutlined />} size="large">Advanced Filters</Button>
              <Button type="primary" size="large">Add New Tyre</Button>
            </Space>
          </div>
          
          {/* Tyres table */}
          <Card bodyStyle={{ padding: 0 }}>
            <Table
              dataSource={filteredTyres}
              columns={columns}
              rowKey="id"
              pagination={{ 
                pageSize: 10,
                showSizeChanger: true,
                showTotal: (total) => `Total ${total} items`
              }}
              scroll={{ x: 'max-content' }}
            />
          </Card>
        </TabPane>
        
        <TabPane tab="Site Inventory" key="2">
          <Row gutter={[16, 16]}>
            {siteInventory.map((site, index) => (
              <Col xs={24} md={8} key={index}>
                <Card 
                  title={
                    <div className="flex justify-between items-center">
                      <span>{site.site}</span>
                      <Badge count={site.total} style={{ backgroundColor: '#1677ff' }} />
                    </div>
                  } 
                  className="mb-4"
                >
                  <Statistic 
                    title="Total Value" 
                    value={site.totalValue} 
                    prefix="$"
                    precision={0}
                    className="mb-4"
                  />
                  
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      <Text strong>In Service:</Text> {site.inService}
                    </div>
                    <div>
                      <Text strong>In Stock:</Text> {site.inStock}
                    </div>
                    <div>
                      <Text strong>Needs Replacement:</Text> {site.needsReplacement}
                    </div>
                    <div>
                      <Text strong>Under Repair:</Text> {site.underRepair}
                    </div>
                  </div>
                  
                  <div className="flex justify-end">
                    <Button type="primary" size="small">
                      View Details
                    </Button>
                  </div>
                </Card>
              </Col>
            ))}
          </Row>
          
          <Card title="Inventory Distribution" className="mb-4">
            <Row gutter={[16, 16]}>
              <Col xs={24} md={12}>
                <Card title="Tyres by Status" bordered={false}>
                  <div className="mb-2">
                    <div className="flex justify-between">
                      <Text>In Service</Text>
                      <Text>{inventoryStats.inService}</Text>
                    </div>
                    <Progress 
                      percent={Math.round(inventoryStats.inService / inventoryStats.total * 100)} 
                      strokeColor="#52c41a" 
                    />
                  </div>
                  <div className="mb-2">
                    <div className="flex justify-between">
                      <Text>In Stock</Text>
                      <Text>{inventoryStats.inStock}</Text>
                    </div>
                    <Progress 
                      percent={Math.round(inventoryStats.inStock / inventoryStats.total * 100)} 
                      strokeColor="#1677ff" 
                    />
                  </div>
                  <div className="mb-2">
                    <div className="flex justify-between">
                      <Text>Under Repair</Text>
                      <Text>{inventoryStats.underRepair}</Text>
                    </div>
                    <Progress 
                      percent={Math.round(inventoryStats.underRepair / inventoryStats.total * 100)} 
                      strokeColor="#faad14" 
                    />
                  </div>
                  <div className="mb-2">
                    <div className="flex justify-between">
                      <Text>Needs Replacement</Text>
                      <Text>{inventoryStats.needsReplacement}</Text>
                    </div>
                    <Progress 
                      percent={Math.round(inventoryStats.needsReplacement / inventoryStats.total * 100)} 
                      strokeColor="#ff4d4f" 
                    />
                  </div>
                </Card>
              </Col>
              <Col xs={24} md={12}>
                <Card title="Value Distribution" bordered={false}>
                  <div className="mb-2">
                    <div className="flex justify-between">
                      <Text>Active Value</Text>
                      <Text>${inventoryStats.activeValue.toLocaleString()}</Text>
                    </div>
                    <Progress 
                      percent={Math.round(inventoryStats.activeValue / inventoryStats.totalValue * 100)} 
                      strokeColor="#52c41a" 
                    />
                  </div>
                  <div className="mb-2">
                    <div className="flex justify-between">
                      <Text>Inactive Value</Text>
                      <Text>${inventoryStats.inactiveValue.toLocaleString()}</Text>
                    </div>
                    <Progress 
                      percent={Math.round(inventoryStats.inactiveValue / inventoryStats.totalValue * 100)} 
                      strokeColor="#1677ff" 
                    />
                  </div>
                </Card>
              </Col>
            </Row>
          </Card>
        </TabPane>
      </Tabs>
      
      {/* Tyre details drawer */}
      {selectedTyre && (
        <Drawer
          title={`Tyre Details: ${selectedTyre.serial}`}
          placement="right"
          onClose={() => setDrawerVisible(false)}
          open={drawerVisible}
          width={600}
        >
          <Tabs defaultActiveKey="1">
            <TabPane tab="Information" key="1">
              <Descriptions bordered column={1} className="mb-4">
                <Descriptions.Item label="Serial Number">{selectedTyre.serial}</Descriptions.Item>
                <Descriptions.Item label="Brand / Model">{selectedTyre.brand} {selectedTyre.model}</Descriptions.Item>
                <Descriptions.Item label="Size">{selectedTyre.size}</Descriptions.Item>
                <Descriptions.Item label="Current Location">{selectedTyre.location}</Descriptions.Item>
                <Descriptions.Item label="Status">
                  <Tag color={getStatusColor(selectedTyre.status)}>{selectedTyre.status}</Tag>
                </Descriptions.Item>
                <Descriptions.Item label="Purchase Date">{selectedTyre.purchaseDate}</Descriptions.Item>
                <Descriptions.Item label="Cost">${selectedTyre.cost.toLocaleString()}</Descriptions.Item>
                {selectedTyre.machine && (
                  <Descriptions.Item label="Machine">{selectedTyre.machine}</Descriptions.Item>
                )}
                {selectedTyre.position && (
                  <Descriptions.Item label="Position">{selectedTyre.position}</Descriptions.Item>
                )}
                {selectedTyre.installDate && (
                  <Descriptions.Item label="Installation Date">{selectedTyre.installDate}</Descriptions.Item>
                )}
                <Descriptions.Item label="Tread Depth">
                  <Progress 
                    percent={selectedTyre.treadDepth} 
                    status={selectedTyre.treadDepth < 20 ? "exception" : undefined}
                    strokeColor={getConditionColor(selectedTyre.treadDepth)}
                  />
                </Descriptions.Item>
                {selectedTyre.hours > 0 && (
                  <Descriptions.Item label="Hours in Operation">{selectedTyre.hours}</Descriptions.Item>
                )}
              </Descriptions>
              
              <div className="flex justify-end mt-4">
                <Space>
                  <Button>Schedule Inspection</Button>
                  <Button>Transfer Location</Button>
                  <Button type="primary">Update Status</Button>
                </Space>
              </div>
            </TabPane>
            
            <TabPane tab="History" key="2">
              <List
                itemLayout="horizontal"
                dataSource={selectedTyre.history}
                renderItem={(item: any) => (
                  <List.Item>
                    <List.Item.Meta
                      avatar={<Avatar icon={<CaretRightOutlined />} />}
                      title={<span><Text strong>{item.date}</Text> - {item.event}</span>}
                      description={item.details}
                    />
                  </List.Item>
                )}
              />
            </TabPane>
            
            <TabPane tab="Financial" key="3">
              <Card title="Cost Analysis" className="mb-4">
                <Row gutter={[16, 16]}>
                  <Col span={12}>
                    <Statistic 
                      title="Purchase Cost" 
                      value={selectedTyre.cost} 
                      prefix="$" 
                      precision={0} 
                    />
                  </Col>
                  <Col span={12}>
                    <Statistic 
                      title="Current Value" 
                      value={selectedTyre.cost * (selectedTyre.treadDepth / 100)} 
                      prefix="$" 
                      precision={0} 
                    />
                  </Col>
                  <Col span={12}>
                    <Statistic 
                      title="Depreciation" 
                      value={selectedTyre.cost * (1 - selectedTyre.treadDepth / 100)} 
                      prefix="$" 
                      precision={0} 
                    />
                  </Col>
                  <Col span={12}>
                    <Statistic 
                      title="Cost per Hour" 
                      value={selectedTyre.hours > 0 ? selectedTyre.cost / selectedTyre.hours : 0} 
                      prefix="$" 
                      precision={2} 
                    />
                  </Col>
                </Row>
              </Card>
            </TabPane>
          </Tabs>
        </Drawer>
      )}
    </div>
  )
} 