...
export default function DirectorLayout ({
  ...
  const generateBreadcrumbs = () => {
    ...
    breadcrumbItems.push({
      title: <Link to='/director'>Dashboard</Link>
    })
    ...
    if (paths.length > 2) {
      const subPageTitle =
        paths[2]?.charAt(0).toUpperCase() +
        paths[2]?.slice(1).replace(/-/g, ' ')
      breadcrumbItems.push({ title: subPageTitle })
    }
            ...
            <Space>
              ...
              >
                <div className='flex items-center cursor-pointer ml-4'>
                  ...
                  />
                  {!collapsed && (
                    <span className='ml-2'>{userData?.name || 'Director'}</span>
                  )}
                </div>
              </Dropdown>
            </Space>
